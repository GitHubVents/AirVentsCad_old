    ...
    public partial class ModelSw
    {
        ...
        public string DumperS(string typeOfFlange, string width, string height, bool isOutDoor)
        {
            ...
            if (typeOfFlange == "20")
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        NewComponentsFull.Add(new VentsCadFiles { LocalPartFileInfo = new FileInfo(
                    ...
                    NewComponentsFull.Add(new VentsCadFiles { LocalPartFileInfo = new FileInfo(newPartPath).FullName });
                    NewComponentsFull.Add(new VentsCadFiles { LocalPartFileInfo = new FileInfo($@"{Settings.Default.DestinationFolder}\{DamperDestinationFolder}\{newNameAsm}.SLDDRW").FullName });
                }
            }
            ...
            if (typeOfFlange == "30")
            {
                ...
                NewComponentsFull.Add(new VentsCadFiles { LocalPartFileInfo = new FileInfo(newPartPathAsm).FullName });
                NewComponentsFull.Add(new VentsCadFiles { LocalPartFileInfo = new FileInfo($@"{Settings.Default.DestinationFolder}\{DamperDestinationFolder}\{newNameAsm}.SLDDRW").FullName });
            ...
            NewComponentsFull.Add(new VentsCadFiles { LocalPartFileInfo = new FileInfo(name + ".SLDASM").FullName });
            NewComponentsFull.Add(new VentsCadFiles { LocalPartFileInfo = new FileInfo(name + ".SLDDRW").FullName });
        ...
        public void Roof(string type, string width, string lenght)
        {
            ...
            //           Path.GetFileNameWithoutExtension(new FileInfo(path).FullName)), MessageBoxButton.YesNoCancel) == MessageBoxResult.Yes)
            //{
            //    LoggerError(string.Format("Генерация крыши: тип - {0}, ширина - {1}, высота - {2}", typeOfFlange, width, lenght), "", "Roof");
            //    RoofStr(typeOfFlange, width, lenght, false);
            //}
        }
        ...
        string RoofStr(string type, string width, string lenght, bool onlyPath)
        {
            ...
            //    var newPartName = String.Format("15-0{0}-01-{1}-{2}.SLDPRT", typeOfFlange, width, lenght);
